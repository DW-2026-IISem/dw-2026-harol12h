...
export function resolveDialectCredentials(): DatabaseConfig {
  const dialect = (process.env.DB_DIALECT as DatabaseDialect) ?? DatabaseDialect.Postgres;
  ...
  const credentials = credentialsByDialect[dialect as keyof typeof credentialsByDialect];
